...
/**
 * Nudge Store - MODE Framework v2.0 (Web)
 *
...
const isReadyToShow = (nudge: Nudge): boolean => {
  if (nudge.shownAt) return false;
  if (isExpired(nudge)) return false;
  if (nudge.showAt && Date.now() < nudge.showAt) return false;
  return true;
};
...
export const NudgeActions = {
  ...
  updateNote: (noteId: string, changes: Record<string, unknown>): NudgeAction => ({
    type: 'update_note',
    noteId,
    changes,
  }),
