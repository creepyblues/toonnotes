/**
 * Behavior Store - MODE Framework v2.0
 *
 * Tracks note behaviors and engagement patterns within the MODE framework.
 * Each note has associated behavior data that tracks:
 * - Usefulness lifecycle progression
 * - Engagement metrics (access count, edit count)
 * - Mode-specific data (ManageData, DevelopData, etc.)
...
import {
  DevelopContentType,
  DevelopData,
  DevelopMaturityLevel,
  ExperienceData,
  ExperienceDepthLevel,
  ManageData,
  ManageUsefulnessLevel,
  Mode,
  ModeData,
  NoteBehavior,
  OrganizeData,
...
const calculateManageScore = (data: ManageData): number => {
  ...
  // Captured (base): 10 points
  score += 10;

  // Has deadline: +25 points
  if (data.hasDeadline) score += 25;

  // Has priority: +20 points
  if (data.hasPriority) score += 20;

  // Has subtasks: +15 points
  if (data.hasSubtasks) score += 15;

  // Completed: +30 points (max)
...
const calculateDevelopScore = (data: DevelopData): number => {
  ...
  // Bonus for expansions (up to +15)
  score += Math.min(data.expansionCount * 5, 15);

  // Bonus for linked ideas (up to +10)
...
const calculateOrganizeScore = (data: OrganizeData): number => {
  ...
  // Bonus for tags (up to +10)
  score += Math.min(data.tags.length * 2, 10);

  // Bonus for usage (up to +20)
  score += Math.min(data.usageCount * 4, 20);

  // LEARN stage bonus for mastery
  if (data.stage === 'learn' && data.masteryLevel) {
    score += Math.floor(data.masteryLevel * 0.2);
  }
...
const calculateExperienceScore = (data: ExperienceData): number => {
  ...
  // Bonus for location (+5)
  if (data.hasLocation) score += 5;

  // Bonus for people tagged (up to +10)
...
interface BehaviorState {
  ...
  addStateTransition: (noteId: string, from: string, to: string, trigger?: string) => void;

  // Actions - Usefulness calculation
  recalculateScore: (noteId: string) => number;

  // Actions - User patterns
    ...
    (set, get) => ({
      ...
      updateBehavior: (noteId, updates) => {
        set((state) => {
          ...
          // Recalculate score if modeData changed
          if (updates.modeData) {
      ...
      trackAccess: (noteId) => {
        set((state) => {
          ...
          // Update organize flow stage if accessed enough
      ...
      updateManageData: (noteId, updates) => {
        ...
        // Update usefulness level based on data
      ...
      updateOrganizeData: (noteId, updates) => {
        ...
        // Update flow stage based on usage
      ...
      updateExperienceData: (noteId, updates) => {
        ...
        // Update depth level based on content richness
      ...
      // State transitions
      addStateTransition: (noteId, from, to, trigger) => {
        const { behaviors, updateBehavior } = get();
        const existing = behaviors[noteId];
        if (!existing || existing.mode !== 'manage') return;
        ...
        updateBehavior(noteId, {
          modeData: {
            ...currentData,
            stateHistory: [...currentData.stateHistory, transition],
          },
        });
      },
