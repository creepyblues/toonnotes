...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('Sticker generation: Using Gemini to remove background...');
...
Output a PNG with the subject on a solid white background.`;
    ...
    if (!stickerData) {
      console.warn('Sticker generation: Gemini did not return an image, falling back to original');
    ...
    console.log('Sticker generation: Converting white background to transparent...');
    const transparentSticker = await convertBackgroundToTransparent(stickerData);

    console.log('Sticker generation: Background removed and made transparent');
