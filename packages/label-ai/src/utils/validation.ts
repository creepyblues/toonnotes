...
export function parseAnalysisResult(data: unknown): ValidatedAnalysisResult {
  ...
  if (!result.success) {
    console.warn('[label-ai] Invalid analysis response, using defaults:', result.error.issues);
    return {
      matchedLabels: [],
      suggestedNewLabels: [],
      analysis: {
...
export function parseGeneratedLabelDesign(data: unknown, labelName: string): ValidatedGeneratedLabelDesign {
  ...
  if (!result.success) {
    console.warn('[label-ai] Invalid label design response, using defaults:', result.error.issues);
