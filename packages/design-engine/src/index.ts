...
export function composeStyle(
  ...
  const preset = design.labelPresetId
    ? getPresetById(design.labelPresetId as LabelPresetId)
    : undefined;
...
export function getNoteDesignFromLabelPresetId(presetId: string): NoteDesign | null {
  const preset = getPresetById(presetId as LabelPresetId);
...
export { CONTEXT_RULES, DARK_MODE_COLORS, DEFAULT_BORDER_RADIUS, DEFAULT_SHADOW };
